...
def on_generation(ga_instance):
    ...
    print(
        f"Fitness    = {-1*ga_instance.best_solution(pop_fitness=ga_instance.last_generation_fitness)[1]}"
    )
    print(
        f"Change     = {-1*ga_instance.best_solution(pop_fitness=ga_instance.last_generation_fitness)[1] - -1*last_fitness}"
    )
...
def ioflexgad():
    ...
    ap.add_argument(
        "--outfile",
        type=str,
        help="Path to output history runs file",
        default="./OutIOFlexGA.csv",
    )
    ap.add_argument(
        "--infile", type=str, help="Path to file from previous runs", default=None
    )
    ap.add_argument(
        "--outgainstance",
        "-o",
        type=str,
        help="Genetic algorithm instance output file",
        default=None,
    )
    ap.add_argument(
        "--cmd",
    ...
    ap.add_argument(
        "--select_type",
        type=str,
        help="GA parent selection type, default=rws",
        default="rws",
    )
    ap.add_argument(
        "--cross_prob",
        type=float,
        help="crossover probability values between 0.0 and 1.0",
        default=0.2,
    )
    ap.add_argument(
        "--mut_prob",
        type=float,
        help="mutation probability values between 0.0 and 1.0",
        default=0.2,
    )
    ap.add_argument("--num_gens", type=int, help="Number of generations", default=40)
    ap.add_argument(
        "--num_parents", type=int, help="Number of parents mating", default=5
    )
    ap.add_argument("--pop_size", type=int, help="Population size", default=10)
    ap.add_argument(
        "--cross_type", type=str, help="crossover type", default="scattered"
    )
